...
public class DesCipher : ISymmetricCipher
{
    ...
    private static BitArray F(BitArray right, BitArray key)
    {
        ...
        for (var i = 0; i < 48; ++i) extendedRight[i] = right[exp[i] - 1];
        ...
        for (var k = 0; k < 8; ++k)
        {
            var i = ((B[k][0] ? 1 : 0) << 1) | (B[k][5] ? 1 : 0);
            var j = (B[k][4] ? 1 : 0) | ((B[k][3] ? 1 : 0) << 1) | ((B[k][2] ? 1 : 0) << 2) | ((B[k][1] ? 1 : 0) << 3);
    ...
    private static readonly int[] ip1 =
    {
    ...
    public void CreateRoundKeys(byte[] key)
    {
        ...
        for (var i = 0; i < 16; ++i) CD.Add(C[i].JoinArr(D[i]));
