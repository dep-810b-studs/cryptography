...
public class BitArray : ICollection<bool>, ICloneable, IEnumerable
{
    ...
    public static class FirstSectionOfOperations
    {
        ...
        {
            var intraNumber = Number;
            var result = ((1 << IndexNumber) & intraNumber) >> IndexNumber;
            return result;
        }
        ...
        {
            var intraNumber = Number;
            var withoutIJbits = intraNumber & (-1 ^ ((1 << i) | (1 << j)));
            Number = (getIbit(i, intraNumber) << j) | (getIbit(j, intraNumber) << i) | withoutIJbits;
        }
    ...
    {
        intraBitArray = new List<bool>();
        Func<byte, List<bool>> transformByteToListBools = element =>
        {
            var tempList = new List<bool>();
            for (var i = 0; i < byteLength; i++) tempList.Add(false);
        ...
        intraBitArray = (from element in inputByteList.AsParallel() select transformByteToListBools(element))
    ...
    public void getBitArrayFromByteArr(List<byte> inputArray)
    {
        ...
        Func<byte, List<bool>> transformByteToListBools = element =>
        {
            ...
            for (var i = 0; i < 8; i++) tempList[i] = FirstSectionOfOperations.getIbit(element, i) == 1 ? true : false;
            tempList.Reverse();
            return tempList;
        };
        inputArray.Reverse();
        intraBitArray = (from element in inputArray.AsParallel() select transformByteToListBools(element))
            .SelectMany(x => x).ToList();
    ...
    {
        var longToByteArr = BitConverter.GetBytes(inputLongValue);
    ...
    /// </summary>
    public int Count => intraBitArray.Count;
    ...
    public void CopyTo(byte[] array, int arrayIndex)
    {
        Parallel.For(0, intraBitArray.Count,
            (i, state) =>
            {
                ...
                {
                    if (intraBitArray[i])
                        FirstSectionOfOperations.SetIbit(7 - (i + arrayIndex) % 8,
                            ref array[array.Count() - 1 - (arrayIndex + i) / 8]);
                    else
                        FirstSectionOfOperations.TakeOffIbit(7 - (i + arrayIndex) % 8,
                            ref array[array.Count() - 1 - (arrayIndex + i) / 8]);
                }
                catch
                {
    ...
    {
        Parallel.For(0, bitArray1.Count, i => { bitArray1[i] ^= bitArray2[i]; });
    ...
    {
        get => intraBitArray[index];
        set => intraBitArray[index] = value;
    }
    ...
    public long getLongValFromBitArray
    {
        get
        {
            var tempRes = new byte[8];
            CopyTo(tempRes, 0);
            return BitConverter.ToInt64(tempRes, 0);
        }
    }

    public char[] elementsOfBitArrayInChar => ToString().ToArray();

    public string getLongHexadecimalFromBitarray => getLongValFromBitArray.ToString("X");

    public string getUintHexadecimalFromBitarray => getUintValFromBitArray.ToString("X");

    public byte[] getByteArrayFromBitArray
    {
        get
        {
            var tempByteArr = new byte[Count / 8];
