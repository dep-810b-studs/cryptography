...
public sealed class RijandelCipher : ISymmetricCipher
{
    ...
    private static readonly byte[] _cPolynomial = { 3, 1, 1, 2 };
    private static readonly byte[] _inversedCPolynomial = { 11, 13, 9, 14 };
    private static readonly byte[] _rCon = { 1, 2, 4, 8, 16, 32, 64, 128, 27, 54 };
    ...
    {
        var shifts = new[] { Mode.Shifts.C1, Mode.Shifts.C2, Mode.Shifts.C3 };
    ...
    private void MixColumns(byte[] state, CipherAction cipherAction)
    {
        ...
        var numArray = new byte[blockSize];
        for (var column = 0; column < Mode.Nb; ++column)
