...
namespace Cryptography.Algorithms.DES
{
    class DES
    {
        ...
        public static BitArray[] DecryptBlocks(BitArray[] blocks, BitArray[] keys)
        {
            ...
            {
                if(block.Count < 64)
                    block.IncreaseLength(64 - block.Count);
                res.Add(DeCrypt(block, keys));
            }
            return res.ToArray();
        }

        public static BitArray[] EnCryptBlocks(BitArray[] blocks, BitArray[] keys)
        {
            var res = new List<BitArray>();
            ...
            foreach (var block in blocks)
            {
                if (block.Count < 64)
                    block.IncreaseLength(64 - block.Count);
                res.Add(EnCrypt(block, keys));
            }
            return res.ToArray();
        }

        public static BitArray StringKeyToBitArray(string key)
        {
            var keyInBytes = Encoding.ASCII.GetBytes(key);

            if (keyInBytes.Length > 8) throw new Exception("Неверный размер ключа");

            var keyInBitArr = new BitArray(8);
            var tempKey = new byte[8];

            for (int i = 0; i < keyInBytes.Length; i++)
                tempKey[i] = keyInBytes[i];

            var tmp = new List<byte>(tempKey);

            tmp.Reverse();

            keyInBytes = tmp.ToArray();

            keyInBitArr.getBitArrayFromByteArr(new List<byte>(keyInBytes));

            return keyInBitArr;
        }
        public static BitArray[] StringToBitArrays(string msg)
        {
            var msgInBytes = Encoding.ASCII.GetBytes(msg);
            var tempMsgInBits = new List<BitArray>();

            int countPartsOfMessage = msgInBytes.Length / 8;

            var tempByteArr = new  List<byte>();

            for (int i = 0; i < msgInBytes.Length; i++)
            {
                if(((i+1)%8==0)||(i == msgInBytes.Length-1))
                {
                    var temp = new BitArray(tempByteArr);
                    tempMsgInBits.Add(temp);
                    tempByteArr = new List<byte>();
                }

                tempByteArr.Add(msgInBytes[i]);
            }

            return tempMsgInBits.ToArray();

        }

        #region Modes

        public static BitArray[] EnCryptECB(string msg, string key)
        {
            var msgInBytes = Encoding.ASCII.GetBytes(msg);

            var blocks = StringToBitArrays(msg);
            var keyy = StringKeyToBitArray(key);

            var keys = Key(keyy);

            var encrypted = EnCryptBlocks(blocks, keys);

            return encrypted;
        }
        public static string DeCryptECB(BitArray[] encrypted, string key)
        {
            var keyy = StringKeyToBitArray(key);

            var keys = Key(keyy);

            var decrypted = DecryptBlocks(encrypted, keys);

            var templist = new List<BitArray>(decrypted);

            templist.Reverse();

            decrypted = templist.ToArray();

            foreach (var block in decrypted)
                    block.removeNullsfromBegin();

            return Encoding.ASCII.GetString(BitArray.ConcatArr(decrypted).getByteArrayFromBitArray);

            }

        public static BitArray[] EnCryptCBC(string msg, string key, byte[] IV)
        {
            var IvInBits = new BitArray(IV);
            ...
            foreach (var block in blocks)
            {
                if (block.Count < 64)
                    block.IncreaseLength(64 - block.Count);
                var tempBlock = EnCrypt(block ^ IvInBits, keys);
                res.Add(tempBlock);
                IvInBits = tempBlock.Clone() as BitArray;
            }

            return  res.ToArray();
        }

        public static string DeCryptCBC(BitArray[] encrypted, string key, byte[] IV)
        {
            var IvInBits = new BitArray(IV);

            var c = new BitArray(IV);
            ...
            foreach (var block in encrypted)
            {
                if (block.Count < 64)
                    block.IncreaseLength(64 - block.Count);
                var temp = block.Clone() as BitArray;

                var tempBlock = DeCrypt(block, keys);
                res.Add(tempBlock ^ IvInBits);
            ...
            return Encoding.ASCII.GetString(BitArray.ConcatArr(res.ToArray()).getByteArrayFromBitArray);
        }

        #endregion
        public static BitArray DeCrypt(BitArray msgInBitArr, BitArray[] keys)
        {
        ...
        public static BitArray EnCrypt(BitArray msgInBitArr, BitArray[] keys)
        {
            msgInBitArr.applyPermutations(ip);
        ...
        public static BitArray F(BitArray right, BitArray key)
        {
        ...
        public static BitArray[] Key(BitArray keyInBitArr)
        {
