    ...
    public class GaloisFieldJobs : BaseJobs
    {
        ...
        private void MultiplicationInGF256Job()
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine($"0x{Convert.ToString(firstNumber, 16)} * 0x{Convert.ToString(secondNumber, 16)} = 0x{Convert.ToString(multiplicationResult,16)}");
        ...
        private void ExtendedEclidianAlgorithmJob()
        {
            while (true)
            {
                ...
                Console.WriteLine($"0b{Convert.ToString(firstNumber.Value, 2)} *  0b{Convert.ToString(x.Value, 2)} + 0b{Convert.ToString(secondNumber.Value, 2)} * 0b{Convert.ToString(y.Value, 2)} = 0b{Convert.ToString(gcd.Value,2)}");
                Console.WriteLine($"0x{Convert.ToString(firstNumber.Value, 16)} *  0x{Convert.ToString(x.Value, 16)} + 0x{Convert.ToString(secondNumber.Value, 16)} * 0x{Convert.ToString(y.Value, 16)} = 0x{Convert.ToString(gcd.Value,16)}");
                Console.WriteLine("Для продолжения операций нажмите любую клавишу. Для выхода нажмите q");
                var userChoice = Console.ReadLine();
        ...
        private void FindInverseJob()
        {
            while (true)
            {
                var irreduciblePolynmial = GetNumberFromUser("Поиск обратного в GF256. Введите неприводимый полином для поля GF256:");
                var number = (byte)GetNumberFromUser("Введите многочлен(в десятичной СС)", "byte");

                _galoisField.IrreduciblePolynomial = irreduciblePolynmial;
                
                var methodExecutionWatch = System.Diagnostics.Stopwatch.StartNew();
                var inverseUsingPotential = _galoisField.MultiplicativeInverse(number, MultiplicativeInverseCalculationWay.Exponentiation);
                methodExecutionWatch.Stop();
                Console.WriteLine($"Exponentiation. {number}^-1 = {inverseUsingPotential}. Time: {methodExecutionWatch.ElapsedMilliseconds} ms");

                methodExecutionWatch.Restart();
                var inverseUsingGcdExt = _galoisField.MultiplicativeInverse(number, MultiplicativeInverseCalculationWay.ExtendedEuclideanAlgorithm);
                methodExecutionWatch.Stop();
                Console.WriteLine($"ExtendedEuclideanAlgorithm. {number}^-1 = {inverseUsingGcdExt}. Time: {methodExecutionWatch.ElapsedMilliseconds} ms");
